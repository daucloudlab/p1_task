...
def trainTest(classifier, X, Y, Xtest, Ytest):
    ...
    print ("Training accuracy %g, test accuracy %g" % (trAcc, teAcc)) #өзгертілді
...
def learningCurve(classifier, X, Y, Xtest, Ytest):
    ...
    for i in range(1, M+1):    # loop over "skip lengths"
        ...
        print ("Training classifier on %d points..." % ids.size) #өзгертілді
...
def hyperparamCurve(classifier, hpName, hpValues, X, Y, Xtest, Ytest):
    ...
    for m in range(M):
        # report what we're doing
        print ("Training classifier with %s=%g..." % (hpName, hpValues[m])) #Өзгертілді
