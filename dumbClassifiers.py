...
class AlwaysPredictMostFrequent(BinaryClassifier):
    ...
    def train(self, X, Y):
        ...
        ### TODO: YOUR CODE HERE
        ### util.raiseNotDefined()
        unique_items, counts = unique(Y, return_counts=True)
        max_index = counts.argmax()
        # print(unique_items, counts, max_index)
        self.mostFrequentClass = unique_items[max_index]
...
class FirstFeatureClassifier(BinaryClassifier):
    ...
    def train(self, X, Y):
        ...
        for i in t:
            if i > 0:
                when_x_positiv += Y[index]
            else:
                when_x_negative += Y[index]
